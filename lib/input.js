...
const { basename, extname, join, sep, resolve } = require('path')
const fs = require('fs')
 ...
 * @TODO should give it a pretty name instead of a random one?
...
function moveBin(pathToBin) {
  try {
    if (fs.existsSync(APP_PATH)) {
      const binName = basename(pathToBin)
      const dest = join(APP_PATH, binName)

...
const { getBinPath, moveBin, cleanUp } = require('./input')
 ...
 * @return {promise} resolve the final executed result
 */
function run(pathToFile, params, options) {
  ...
  return compile(pathToFile, params)
          .then(pathToBin => {
            const result = exec(pathToBin, args)
            // console.info('exec result', result)
            return [result, pathToBin]
          })
          .then(t => {
            const [result, pathToBin] = t
